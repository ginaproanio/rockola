const CLOUD_NAME = 'dxq0bnzqd'
const UPLOAD_PRESET_SONGS = 'rockola_songs'
const UPLOAD_PRESET_COVERS = 'rockola_covers'
...
export const cloudinaryService = {
  uploadAudio: async (file: File): Promise<string> => {
    try {
      ...
      formData.append('upload_preset', UPLOAD_PRESET_SONGS)
  ...
  uploadCover: async (file: File): Promise<string> => {
    try {
      const formData = new FormData()
      formData.append('file', file)
      formData.append('upload_preset', UPLOAD_PRESET_COVERS)
